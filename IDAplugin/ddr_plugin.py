...
#
#   Usage and installation details can be found here: 
...
DDR_WEBAPI_KEY              = "UZYCHQANPR52H3VVDC7EPR71"  # gets generated by the ddr_server.py script, check its output.
# CA certificate
CA_CERT                     = r"C:\Users\Dex Dexter\Documents\DDR_tools\ddr_server.crt"   
# Verify certificates
VERIFY_CERT                 = True
...
# ddr_plugin.py hands this directory over to ddr_server.py via the API call, the ddr_server.py script using it then 
# as a parameter for launching the DynamoRIO client DLL
# In this alpha version you need to copy the sample you are analysing in IDA manually into this directory before 
# running a trace.
SERVER_LOCAL_SAMPLE_DIR     = r"C:\Users\Dex Dexter\Documents\DDR_samples" 
...
class DDR_ida_action_handler(idaapi.action_handler_t):
		...
		for c,i in enumerate(trace_instr_num_list):
			if c < MAX_LOG_ROUNDS:
				try:
					...
					DDR_print_mesg("    %s@instr number:%07d = not found in data. Did you run a full trace ?" % (reg,i))
			if c < MAX_CMT_ROUNDS:
				s_pdata  = "NO_DATA"
				regval   = 0
	...
	def _test_zip_is_valid(self, zipfilename):
		...
		try:
			...
			if res is not None:
				DDR_print_mesg("Bad file in zip archive: %s" % res, 2)
				return False
			else:
				return True
		except:
			DDR_print_mesg("Zip file: %s is corrupt. File content:\n" % zipfilename, 2)

			with open(zipfilename, 'r') as f:
				print f.read()

			os.remove(zipfilename)
			DDR_print_mesg("Corrupt zip file: %s deleted." % zipfilename, 2)
			return False
			...
			if cmd_id == 1: 
				...
				# ...if not, start analysis:
					...
					if res.status_code == 200:
						...
						DDR_print_mesg("API call failed. Respondscode=%d API status:%s" 
									% (res.status_code, res.json()["return_status"]))
			...
			if cmd_id == 3: 
				...
				if os.path.exists(jsonfile_name_light_trace_only):
					...
					if res.status_code == 200:
						...
						# Verify zip file is valid
						if not self._test_zip_is_valid(zipfilename):
							DDR_print_mesg("Zip file verification failed for light trace", 2)
							return
						# Extract files from zip archive
						with zipfile.ZipFile(zipfilename, "r") as ziparchive:
							DDR_print_mesg("Extracting files to %s" % jsonfile_dir)
...
class ddrPlugin(idaapi.plugin_t):
	...
	def init(self):
		...
		if not os.path.isfile(CA_CERT):
			DDR_print_mesg("CA certificate file not found. Pls check the CA_CERT variable in the ddr_plugin.py script. Fix it and restart IDA.",3)
			idaapi.warning("[DDR] CA file not found. Pls check the CA_CERT variable in the ddr_plugin.py script. Fix it and restart IDA.")
			return idaapi.PLUGIN_SKIP
