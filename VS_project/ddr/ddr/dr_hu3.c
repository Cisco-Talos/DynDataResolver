...
void usage()
{
	...
	dr_printf("-t                      Light trace only, just log control flow instructions in detail (e.g. call,jmp,..)\n");
...
#ifdef X86_64
	dr_printf("\nDDR Client64 Version 0.1 alpha initializing...\n");
#else
	dr_printf("\nDDR Client32 Version 0.1 alpha initializing...\n");
#endif
...
static void logInstrOpnds(file_t f, instr_t *instr, dr_mcontext_t *mc, void *drcontext, size_t numbytes) {
...
static void __cdecl process_instr(app_pc instr_addr)
{
	...
	if ((instr_addr_fixed >= from_addr) && (instr_addr_fixed <= to_addr)) {
		...
		if (light_trace_only) {
			...
			logInstrOpnds(f, &instr, &mc, drcontext, 16);
			dr_fprintf(f, " },\n");
			inst_count--;
